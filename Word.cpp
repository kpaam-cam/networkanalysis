...
const int FRetro = 5;
	const fval ret = 100;
//	const fval nrt = 0;
const int FHightone = 6;
	const fval htn = 100;
//	const fval nht = 0;
const int FLowtone = 7;
	const fval ltn = 100;
//	const fval nlt = 0;
const int FRising = 8;
	const fval rsg = 100;
//	const fval nrs = 0;
const int FFalling = 9;
	const fval flg = 100;
//	const fval nfl = 0;
const int FLat  = 7;
	const fval lat = 100;
...
void Word::modify( FP p, char c )
{
    switch ( c )
    {
        ...
        case 'X': p[FRetro] = ret;      // "retrofleX" added by SD 8-31-06
            break;
        case 'G': p[FHightone] = htn;    // "hiGhtone" added by CH 4-28-21
            break;
        case 'W': p[FLowtone] = ltn;    // "LoWtone" added by CH 4-28-21
            break;
        case 'U': p[FRising] = htn;     // "rising" added by CH 4-28-21
            break;
        case 'Q': p[FFalling] = ltn;    // "falling" added by CH 4-28-21
            break;
        default:
...
int ladDist( FP p1, FP p2, int vec[12] )
{
    ...
    if ( vowel(p1) && vowel(p2) )
    {
        ...
        //Consider(FHightone);
        dist += diff(FHightone) * vec[FHightone];
        //Consider(FLowtone);
        dist += diff(FLowtone) * vec[FLowtone];
        //Consider(FRising);
        dist += diff(FRising) * vec[FRising];
        //Consider(FFalling);
        dist += diff(FFalling) * vec[FFalling];
        //Consider(FHigh);
